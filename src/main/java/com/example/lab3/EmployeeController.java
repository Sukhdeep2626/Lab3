...
public class EmployeeController implements Initializable {
    ...
    @FXML
    private TableColumn<Employee, String> position; // Changed from doctor to position
    @FXML
    private TableColumn<Employee, Integer> salary;  // Changed from room to salary
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        position.setCellValueFactory(new PropertyValueFactory<Employee, String>("position")); // Updated
        salary.setCellValueFactory(new PropertyValueFactory<Employee, Integer>("salary"));     // Updated
    ...
    public void populateTable() {
        ...
        try (Connection connection = DriverManager.getConnection(jdbcUrl, dbUser, dbPassword)) {
            ...
            while (resultSet.next()) {
                ...
                int monthlySalary = resultSet.getInt("salary");       // Updated
                int yearlySalary = monthlySalary * 12;
                tableView.getItems().add(new Employee(id, name, position,yearlySalary )); // Updated
            }
    ...
    public void InsertData(ActionEvent actionEvent) {
        ...
        try (Connection connection = DriverManager.getConnection(jdbcUrl, dbUser, dbPassword)) {
            ...
            String query = "INSERT INTO tbl_Employee(name, position, salary) VALUES ('" + getName + "','" + getPosition + "','" + getSalary + "')"; // Updated
            Statement statement = connection.createStatement();
            statement.execute(query);
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public void UpdateData(ActionEvent actionEvent) {
        ...
        try (Connection connection = DriverManager.getConnection(jdbcUrl, dbUser, dbPassword)) {
            ...
            String query = "UPDATE tbl_Employee SET name='" + getName + "', position='" + getPosition + "', salary='" + getSalary + "' WHERE id = '" + getid + "'"; // Updated
            Statement statement = connection.createStatement();
            statement.execute(query);
        } catch (SQLException e) {
            e.printStackTrace();
